// Free‑Text Finder with multi-query expansion
const CONFIG = {
...
const MM_HINTS = [
  ...
  {
    mm: ["ဧည့်စား", "စားသောက်", "ဟင်း", "သုပ်", "ထမင်း"],
    en: ["food", "restaurant", "curry", "salad", "rice"],
  },
  {
    mm: ["ရေညှိထမင်း"],
    en: ["rice salad", "soaked rice", "Myanmar rice salad", "htamin thoke"],
  },
  { mm: ["ဘာဂါ"], en: ["burger", "hamburger"] },
];
...
function expandQueries(raw) {
  ...
  if (!term) {
    ...
    if (hasMM) {
      ...
      ["restaurant", "food", "juice", "noodle", "salad"].forEach((e) =>
        variants.add(`${term} ${e}`)
      );
    }
    if (!hasMM) {
      [
        ...
        "တာိုင်",
        "အိန္ဒိယ",
      ].forEach((mm) => variants.add(`${term} ${mm}`));
    }
  }
...
function search() {
  const raw = $("#dish").value;
  const queries = expandQueries(raw);
  if (!raw)
    $(
      "#suggestBar"
    ).textContent = `Tip: Searching popular nearby: “${queries[0]}”`;
  else
    $("#suggestBar").textContent = `Trying: ${queries
      .map((q) => `“${q}”`)
      .join(" · ")}`;
  ...
  runTextSearches(queries)
    ...
    .catch(() => {
      results = [];
      render([], raw);
      $("#countBar").textContent = "No results.";
    });
}
...
$$(".f").forEach((cb) =>
  cb.addEventListener("change", () => applyFiltersAndRender($("#dish").value))
);
...
function render(list, term) {
  ...
  list.forEach((r) => {
    ...
    const btnNav = node.querySelector(".map");
    btnNav.href = navUrl(r.lat, r.lng);
    const btnSite = node.querySelector(".site");
    if (r.website) {
      btnSite.href = r.website;
    } else {
    ...
    favBtn.addEventListener("click", () => {
      if (FAV.has(favKey)) FAV.delete(favKey);
      else FAV.add(favKey);
      saveFav();
      updateFavUI();
      render(list, term);
...
async function openDetails(rawTerm) {
  ...
  const title = el("h4");
  title.textContent = rawTerm || "Dish details";
  ...
  note.textContent =
    "Tips: If a shop offers this item, ask staff or check menu photos.";
  ...
  if (CONFIG.EDAMAM_APP_ID && CONFIG.EDAMAM_APP_KEY && rawTerm) {
    try {
      ...
      more.textContent = `Est. calories: ${Math.round(
        data.calories || 0
      )} kcal (Edamam approx)`;
      body.append(more);
    } catch (e) {}
