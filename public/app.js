...
// World Food Explorer – single-init, no duplicate globals
(function () {
  // ----- prevent double init if script is accidentally loaded twice -----
  if (window.WFE?.initialized) {
    console.warn('WFE already initialized; ignoring duplicate load');
    return;
  }
  const WFE = { initialized: true };
  window.WFE = WFE;
  ...
  WFE.search = doSearchFlow;
  window.search = doSearchFlow; // for any existing calls

  // ===================== BIND UI (autocomplete + icon/Enter) =====================
  document.addEventListener('DOMContentLoaded', ()=>{
    const input = $('#dish');
    const btnSearch = $('#btnSearch');
    const acList = $('#acList');
    ...
    // click icon / press Enter → search
    btnSearch.addEventListener('click', doSearchFlow);
    input.addEventListener('keydown', e=>{
      if(e.key==='Enter'){
        const sel = acList.querySelector('li[aria-selected="true"]');
        if(sel){ input.value = sel.textContent; acList.hidden = true; }
    ...
    function renderAC(items){
      ...
      items.forEach((t,i)=>{
        const li=document.createElement('li'); li.textContent=t; li.role='option';
        if(i===acIndex) li.setAttribute('aria-selected','true');
        li.addEventListener('mousedown', e=>{ e.preventDefault(); input.value=t; acList.hidden=true; doSearchFlow(); });
        li.addEventListener('mouseenter', ()=>{ acIndex=i; refreshSel(); });
        acList.appendChild(li);
      });
      acList.hidden=false;
